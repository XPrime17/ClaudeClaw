...
- **On success**: notify start, `runAgent(task.prompt)`, send result, compute next run via cron expression, `updateTaskAfterRun()`, reset failure counter (`failureCounts.delete(task.id)`)
- **On failure (CRITICAL)**: increment consecutive failure count, compute backoff delay = `Math.min(BACKOFF_BASE_MS * Math.pow(2, failures - 1), MAX_BACKOFF_MS)`, call `updateTaskAfterRun(task.id, Date.now() + delayMs, "FAILED: ...")` to push `next_run` forward. Send error message to user with attempt count and retry delay. Without this backoff, a failing task stays permanently "due" and fires every 60s poll — in production this caused 354 failures in 4 hours.
- `computeNextRun`: use `cron-parser` → `parseExpression(expr).next().getTime()`
- Backoff schedule: 1min → 2min → 4min → 8min → 16min → 30min (cap). Resets to normal cron timing on success.
...
11. **Scheduler backoff is non-negotiable**: If a scheduled task fails and you don't push `next_run` forward, the task stays permanently "due" (`next_run <= now`). Every 60s poll finds it and fires it again immediately. In production, this caused 354 failures in 4 hours, flooding the user's Telegram with error messages and hammering the Claude Code API. The fix: on failure, compute `next_run = now + min(60s * 2^consecutiveFailures, 30min)` and call `updateTaskAfterRun()`. On success, reset the failure counter. This is not a nice-to-have — without it, any transient API outage turns into a cascade.
